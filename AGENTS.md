...
- BBVA **recorta el concepto a ~12 caracteres** ("TRANSFERENCI", "PAGO SERVICI", "CUPON. ARGEN", "LEY NRO 25.4"). Por eso `bbva.ts` clasifica **primero por el código de operación** del banco (columna "Codigo", tres dígitos, estable entre cuentas; tabla `CODIGOS_BBVA` relevada de 5 archivos reales el 2026-09-15) y recién después por texto, comparando palabra por palabra con tolerancia a recortes (`coincide()`), con vocabulario real del banco (cupones Argencard/Cabal, MAE-ACREDITA, DNET, BTOB, OG-DEBITO DI, FB-PAGO A PR, PLANRG = plan de pagos AFIP, DEBZURICH) y mirando el Detalle para plataformas (DELIVERY HERO). Las categorías **transferencia / efectivo / cheque son neutras** y se resuelven por sentido (crédito → recibida/depósito/depositado; débito → enviada/extracción/pagado), por eso el Resumen por Concepto se agrupa por (concepto, categoría). Interpretaciones confirmadas por el dueño: MAE-ACREDITA = cobros con tarjeta; PAGO A PROVE DB MIN = pagos a proveedores; DNET CREDITO = PedidosYa (Delivery Hero). Supuestos propios, a confirmar: PAGO VISA-IN (515) = pago del resumen de la tarjeta; PAGO CON VIS (522) = compra con Visa Débito; INTERES COBR (026) = intereses por descubierto. Resultado con los 5 archivos reales: 0 movimientos en "Otros". Santander/Comafi/MP siguen validados solo con archivos sintéticos.

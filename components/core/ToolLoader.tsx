...
import { herramientas, type OpcionesVariante, type PropsHerramienta } from "@/lib/tools-registry";
...
/**
 * Mapa slug de herramienta → componente "perezoso".
 *
 ...
 */
const componentesPorSlug: Record<string, ComponentType<PropsHerramienta>> = Object.fromEntries(
  herramientas.map((h) => [
    h.slug,
...
interface ToolLoaderProps {
  /** Slug de la HERRAMIENTA (no de la variante). */
  slug: string;
  /** Valores preconfigurados por la variante, si la página es una variante. */
  opciones?: OpcionesVariante;
}

export function ToolLoader({ slug, opciones }: ToolLoaderProps) {
  const Herramienta = componentesPorSlug[slug];
  if (!Herramienta) return null;
  return <Herramienta opciones={opciones} />;
}
